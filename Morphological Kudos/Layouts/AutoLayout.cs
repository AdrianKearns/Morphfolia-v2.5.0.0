...
namespace Morphological.Kudos.Layouts
{
	...
	/// </summary>
    [IsLayoutWebControl]
    public class AutoLayout : BasePageLayout, INamingContainer
	{
            ...
            if (CustomProperties != null)
            {
                for (int i = 0; i < CustomProperties.Count; i++)
                {
                    ...
                    switch (propertyKey)
                    {
                        ...
                        case "NotesFilter":
                            ContentInfoCollection contentCollection = Morphfolia.PublishingSystem.ContentPublishingHelper.GetContentByNotesFilter(propertyValue);
                            for (int c = 0; c < contentCollection.Count; c++)
                            {
                                AddContentToContentContainer(tdR2, contentCollection[c].ContentEntryFilter, contentCollection[c].Content);
                            }
        ...
        [IsCustomProperty,
        PropertyFriendlyName("Content Notes-Filter"),
        PropertyDescription("Allows you to select content by wild-card."),
        PropertySuggestedUsage("Book%")]
